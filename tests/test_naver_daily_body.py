...
class RoutineDocTest(unittest.TestCase):
    def test_the_shared_routine_doc_tells_the_routine_to_stop_writing_it(self) -> None:
        ...
        # 2026-09-22부터 가이드·주간·이벤트도 `naver` 절을 쓰지 않는다(사장님: "본진에서는 비공개 처리, 네이버는 한글 주력").
        # 옛 지시("절 4~6·1,200~3,000자 … 계속 씁니다")가 지시문으로 남아 있으면 안 된다 — 이력 설명에 수치가 나오는 것은 괜찮다.
        self.assertNotIn("`naver` 절을 계속 씁니다", text)
        self.assertIn("2026-09-22", text)
        self.assertIn("한국어 글은 전부", text)
