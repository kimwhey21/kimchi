...
`+N곳`은 다른 매체 N곳이 같은 기사를 썼다는 뜻입니다 — 그만큼 화제라는 신호입니다. 영어 제목은 접지
않으므로(잡지 「시장 읽기」가 그 기사로 글을 씁니다) 첫 줄의 `지수 시세 보도 0건 접음`이 정상입니다.

**피드가 막힌 날·레이더가 빈 날**(2026-09-14): 레이더 끝줄에 `못 읽은 피드:`가 찍히거나 걸리는
제목이 없으면 거기서 멈추지 말고 **WebSearch로 직접 찾습니다**(그 매체 이름 + 주제어, 또는
`site:` 우회를 그 자리에서 만들어 봅니다). 레이더는 한 화면으로 훑는 도구일 뿐입니다.
