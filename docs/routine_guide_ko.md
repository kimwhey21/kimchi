...
4. 형식 예시: `editorial/guides/ko_kospi-kosdaq-difference.json`(2026-09-22 전에는 네이버용 본문이 따로 있었다 — 이제 안 쓴다).
...
레이더 첫 줄의 `지수 시세 보도 N건 접음`은 오류가 아닙니다 — 개장·마감마다 매체 열 곳이 쓰는 "코스피가
몇 % 움직였다" 기사를 접은 것입니다(그 이야기는 시황 글이 씁니다). `+N곳`은 **다른 매체 N곳이 같은 기사를
썼다**는 뜻이라 그 자체가 신호입니다 — 여러 곳이 함께 쓴 이야기가 그날의 화제입니다. 접은 것까지 보려면 `--raw`.

**피드가 막힌 날·레이더가 빈 날**(2026-09-14, 사용자: "rss가 안되면 직접 찾아보고 조사하면 안되니?"):
레이더 끝줄에 `못 읽은 피드:`가 찍히거나 고른 갈래에 걸리는 제목이 없으면, 거기서 멈추지 말고
**WebSearch로 직접 찾습니다** — 그 매체 이름과 주제어(예: `한국경제 배당소득세`)로 검색하거나,
`site:` 우회를 그 자리에서 만들어 봅니다(`https://news.google.com/rss/search?q=site:<매체>+<검색어>&hl=ko&gl=KR&ceid=KR:ko`).
레이더는 **한 화면으로 훑는 도구**일 뿐이고, 못 훑은 것을 조사하지 않을 이유는 아닙니다.
그래도 오늘 뉴스에 걸리는 것이 없으면 목록 순서대로 갑니다 — 그것이 정상입니다.
...
4. **원고.** `editorial/guides/ko_<slug>.json`, 기준표와 같은 스키마에:
   ...
   - `radar_origin`: **아침 레이더에서 고른 주제면 그 제목을 그대로** 적습니다(레이더 화면의 줄 하나).
     레이더에 걸리는 것이 없어 목록 순서대로 갔으면 `목록 순서`, 레이더가 비었거나 피드가 다 막혔으면
     `레이더 실패`. 관문이 막습니다(2026-09-14부터). 몇 주 뒤에 **레이더로 고른 글이 더 읽혔는지**를
     세려면 이 한 줄이 있어야 합니다 — 비워 두면 "안 썼다"와 "적기를 잊었다"가 같아 보입니다.
   - `ko.title`: 「제목 문법」 예문집의 꼴 하나, 20~35자, **검색어가 앞쪽에**. 결론을 다 말하지 않습니다.
     가이드는 **내 돈 축**이 잘 맞습니다(2026-09-17, 「더한 축 여섯」) — `250만원 넘으면 165만원 냅니다: 미국주식
     양도세`, `1억을 넣으면 월 얼마: 커버드콜 ETF 계산`. 경제현미경(블로그차트 1위 이력) 제목의 87%가 숫자이고
     그 숫자는 독자 돈으로 환산돼 있습니다.
   ...
   - `sources`: 본문에 이름을 적은 곳을 **주소까지** 적습니다 — 이제 글 아래 「확인한 것」에 링크로
     실립니다(2026-09-15). 그전에는 개수만 세고 화면에 안 나왔습니다.
   - **`직접 확인한 것` 절을 하나 넣습니다**(2026-09-15, 사용자 승인). 설명만 하는 글과 실제로 해 본 글은
     구글이 다르게 봅니다(금융 글에서 특히). **지어내지 않습니다** — 루틴이 실제로 할 수 있는 것만 씁니다:
     공개된 조회 화면(금융감독원 통합연금포털·ISA다모아·KRX 정보데이터시스템·국세청 홈택스 안내)을
     WebFetch로 열어 **거기서 실제로 보이는 항목 이름과 절차**를 순서대로 적고, 막히는 지점(로그인 필요,
     공동인증서 필요, 항목이 두 곳에 나뉘어 있음)을 그대로 적습니다. 증권 계좌 내부 화면처럼 우리가 못 보는
     것은 **보지 않았다고 적거나 빼십시오.** "저도 계좌를 열어 봤습니다" 같은 없는 경험을 쓰면 안 됩니다.
   - `graphics` 2~4장:
...
7. 휴대폰 알림 한 번(PushNotification): `가이드 커밋: <제목> — 몇 분 안에 https://fermata.it.kr/<slug>/ 로 공개되고,
   네이버에는 10분 안에 전문이 올라갑니다.` 건너뛰었거나 실패했을 때도 한 번, 이유와 함께.
...
- **추천은 하지 않되, 비교표는 반드시 넣습니다**(2026-09-15, 사용자 승인). "어디가 좋다"라고 쓰지 않고,
  공개된 수수료·조건·되는 것/안 되는 것을 **표로 나란히** 놓아 독자가 고르게 합니다. 회사 이름을 빼는 것은
  중립이 아니라 **쓸모없음**입니다 — 구글 1페이지에서 우리를 이기는 글은 전부 회사 이름과 숫자를 답니다
  (실측: `how to buy korean stocks` 1위 글이 Schwab $25~40·IBKR 0.05% 미만을 적고, 안 되는 증권사
  다섯 곳까지 이름을 댔습니다. 우리 글은 "증권사마다 다릅니다"로 끝나 13.6위였습니다).
  숫자에는 확인 날짜와 출처를 붙이고, 우리가 못 확인한 항목은 표에 "확인 못 함"이라고 적습니다.
