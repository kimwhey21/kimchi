...
**그다음 아침 레이더로 순서를 바꿉니다**(2026-09-14):
    ...
    python -m scripts.magazine_radar --set ko

한국 매체와 세 가지 질문 묶음(증시·제도·세금·금리·환율)의 최근 36시간 제목이 갈래별로 나옵니다.
**고른 갈래 안에** 오늘 뉴스에 걸린 주제가 있으면 그것을 위에 있는 것보다 **먼저** 씁니다. 없으면
원래 순서대로 갑니다. 갈래를 바꾸지는 않습니다 — 다양성 규칙이 먼저입니다.

왜 이렇게 하나: 가이드는 상시 글이지만 검색은 그날 뉴스를 따라 움직입니다. "정부 ISA 개편안에 계좌
해지 3배"가 뜬 날 `isa-tax-benefit`을 쓰면 같은 글이 몇 배로 읽힙니다(2026-09-14 레이더 실측 제목입니다).
**레이더는 주제를 만들지 않습니다** — 목록에 있는 주제의 순서만 바꿉니다. 레이더에서 본 기사를 근거로
쓰지 않고, 숫자는 언제나 공식 출처 2곳에서 다시 확인합니다.
...
2. **주제.** 「주제 목록」의 고르는 법대로 — 어제와 다른 갈래에서, 그 갈래의 위에서부터
   (`python -m scripts.magazine_radar --set ko`로 오늘 뉴스에 걸린 것이 있으면 그것을 먼저). 이미 쓴 것은 뺍니다.
