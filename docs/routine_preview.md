...
**이 글은 네이버가 사실상 유일한 공개처입니다**(2026-09-15, 사용자 지시). 네이버에는 **본문 전문이
링크 없이** 나가고, 본진(fermata.it.kr)에는 **비공개로만** 올라갑니다 — 같은 글이 두 곳에 공개되면
네이버가 유사문서로 걸러 낼 수 있기 때문입니다. 원고에 `naver` 절을 따로 쓰지 않아도 되고
`ko.narrative`가 그대로 실립니다. 휴대폰으로 읽힌다는 것만 염두에 두고 문단을 2~3문장으로 끊으십시오.
발행 뒤 본진 주소를 열어 보면 404인 것이 정상입니다.
