...
4. **원고.** 기준표와 같은 스키마입니다.
   ...
   - 제목을 짓기 전에 `python -m scripts.recent_titles preview`로 최근 프리뷰 제목을 읽습니다 —
     끝말·꼴이 겹치면 관문이 막습니다(2026-09-09, 같은 틀 반복 금지). **"이번 제목에 넣을 것"**이 알려 주는
     축(시간·질문·독자)을 채우세요(2026-09-17, 「제목 문법」의 「더한 축 여섯」). 프리뷰는 시간 축이
     자연스럽지만 질문·독자 축도 돌려 씁니다 — `오늘 밤 이것 하나만 보세요`, `이 반등, 오늘 밤도 이어질까?`.
