...
(fermata.it.kr)에는 같은 글이 **비공개(`private`)로만** 올라갑니다 — 주소·분류는 그대로지만
사이트맵·목록·피드·익명 접근에서 빠집니다. 네이버에는 요약본이 아니라 **`ko.narrative` 전문**이
...
9. 휴대폰 알림을 한 번 보냅니다: `주말 Checkpoint 커밋: <제목> — 본진에는 몇 분 안에 비공개로 올라가고, 네이버에는 10분 안에 본문 전문이 공개됩니다.`
