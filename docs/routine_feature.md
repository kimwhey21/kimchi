...
(`FERMATA_AUTO_PUBLISH=true`)가 켜져 있어 커밋 즉시 공개됩니다**(사용자 결정 "체크포인트는
자동발행으로") — 그래서 **관문을 통과한 원고만 커밋합니다.** 이 세션은 `--publish`를 쓰지 않고
워드프레스를 직접 만지지 않습니다. 공개는 워크플로가 합니다.
...
3. **제목·소제목.** 규칙 문장이 아니라 **사장님이 고른 예문집**입니다(`docs/editorial-style.md`
   ...
   독자가 할 일·짧은 단정)을 골라 그 어법을 갈아입힙니다. 버린 꼴(긴 서술문, `돌아온 외국인`,
...
10. 휴대폰 알림을 한 번 보냅니다: `주말 Checkpoint 커밋: <제목> — 몇 분 안에 https://fermata.it.kr/<slug>/ 로 공개되고, 네이버에는 20분 안에 요약본이 올라갑니다.`
