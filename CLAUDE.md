...
- **자동 트리거만 바로 공개(`--publish-live`)한다.** 시황은 예약 실행(schedule),
  편집 원고는 원고 커밋(push)이 그 대상이다. 아무도 검수할 수 없는 경로라 사람의
  승인을 기다릴 수 없어서이고, 앞단의 `editorial_quality`(한국어·영어) 검사를
  통과하지 못하면 예외로 멈춰 아무것도 올라가지 않는 것이 안전장치다.
  수동 실행(`workflow_dispatch`)과 손으로 돌리는 실행은 종전대로 임시저장이다.
