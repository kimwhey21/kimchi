...
- **한국장 워치리스트는 코어(고정) + 동적(그날 편입) 2단 구조다.** 코어는
  `config/watchlist_kr.yaml`의 섹터 대표 21종목이고, 여기에 그날 거래대금 상위
  종목이 `source: "dynamic"`으로 붙는다(`src/fetch_movers.py`). 전에는 코어 10개
  뿐이라 2026-09-03처럼 보험·은행이 주도한 날 KB금융·신한지주를 데이터로 담지
  못했다. 동적 편입 기준은 **거래대금**이며 등락률로 바꾸지 말 것 — 등락률
  상위는 써니전자·레버리지 ETN 같은 것들이 올라온다(실측).
- **동적 편입 종목에는 사진을 붙이지 않고**(위 가드), **기준일이 코어와 다르면
  아예 버린다**(`fetch_kr._fetch_dynamic_tier`). 이름도 모르는 종목 하나가 그날
