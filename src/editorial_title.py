...
_REJECTED_SHAPES = (
    ...
    (re.compile(r"\d+일에 (?:갈린다|가른다|갈립니다|정해진다)|시험대|판가름"), "'N월 N일에 갈린다'·'시험대' 같은 날짜 판정 꼴은 버렸습니다. 날짜는 `…전에 볼 것 셋`, `…까지 볼 것`처럼 독자가 할 일과 함께."),
...
def collect_title_issues(title: str, price_data: dict | None = None,
    ...
    if not hook_names(title):
        issues.append(
            ...
            "이유·독자에게 주는 것(`볼 것 셋`, `사면 안 되는 사람`)·대비·기록·시한, 아니면 22자 이하의 짧은 단정입니다. "
