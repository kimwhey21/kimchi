...
- **자동 트리거만 `--publish-live`로 바로 공개한다.** 시황은 예약 실행(schedule),
  편집 원고는 원고 커밋(push)이 그 대상이다. 아무도 검수할 수 없는 경로라 승인을
  기다릴 수 없어서이고, 품질 검사(한국어·영어)를 통과하지 못하면 예외로 멈춰
  아무것도 올라가지 않는 것이 안전장치다. 수동 실행과 손으로 돌리는 실행은 종전대로
  임시저장이며, `publish_draft(status="draft")` 기본값을 바꾸지 않는다.
