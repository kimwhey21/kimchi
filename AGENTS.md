...
이 파일은 이 저장소에서 작업하는 모든 AI 코딩 에이전트(Codex, Claude Code 등)가 공통으로 따라야 할 규칙이다. `CLAUDE.md`와
네 절(「검증 규칙」·「비용/자동 발행 원칙」·「기준표(feature) 글」·「발행 워크플로우」)이 **글자까지 같다** —
`tests/test_rule_files.py`가 확인한다(2026-09-25부터 네 절 전부). 규칙이 생긴 사건·날짜·사장님 말·실측은 `docs/decisions.md`에
같은 절·같은 순서로 있다. `Read` 툴·WebSearch처럼 Claude Code의 도구 이름이 나오면 같은 일을 하는 자기 도구로 읽는다.
...
- **`editorial_facts`가 원고의 숫자를 시세와 대조한다**(`publish_editorial`이 한국어·영어 양쪽에 돌린다). 두 가지를 본다 — 원고에
  ...
  2026-09-26부터 **방향**(숫자에 바로 붙은 부호·등락 낱말이 시세와 반대)과 **남의 숫자**(이름에 붙은 숫자가 다른 종목 값 —
  같은 문장에 이름이 나온 종목·같은 회사 주식·'각각' 문장만 넘긴다)도 막고, 건너뛰기 판정은 그 문장 안에서만 한다. 이 검사를
  고칠 때는 **지난 원고 전부에 다시 돌려 새로 막히는 맞는 글이 없어야** 올린다(`tests/test_editorial_facts_2026_09_26.py`).
...
- **표지 사진은 `config/photo_pool.yaml`에 미리 승인해 둔 것만 쓴다.** 시황은 사람 검수 없이 자동 공개되므로 발행 시점에 검색하면
  ...
  **사진에 실제로 보이는 것**을 적는다.
  ...
  - **동적 편입 종목에는 붙이지 않는다.**
  - 같은 업종에 사진이 여럿이면 **가장 오래 안 쓴 사진**을 고른다(`featured_image.cover_history`가 커밋된 시황 원고에서 이력을
    다시 만든다, 2026-09-26) — 다 쓰기 전에는 같은 사진이 안 돌아오고 보관함에 사진을 더해도 순서가 안 밀린다. 날짜÷장수나
    횟수÷장수로 돌리지 말 것(앞은 닷새 뒤에 같은 사진이 왔고, 뒤는 사진을 더하자 방금 쓴 사진이 또 걸렸다). 새 사진에는 `added`
    날짜를 적는다. 같은 날 다시 돌리면 같은 사진이 나온다(재실행이 표지를 바꾸면 워드프레스에
    미디어가 중복으로 쌓인다).
